...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Source link */}
      <div className="absolute inset-x-0 bottom-[calc(1.5rem+env(safe-area-inset-bottom))] flex justify-center">
        <a
          href="https://github.com/wu-wilson/krawly"
          target="_blank"
          rel="noopener noreferrer"
          className={`inline-flex items-center gap-1.5 text-xs text-text-tertiary
            hover:text-brand transition-colors duration-150 ease-out
            focus:outline-none focus:ring-2 focus:ring-brand/50 rounded px-1.5 py-0.5${shouldAnimate ? ' animate-landing-from-bottom' : ''}`}
          style={shouldAnimate ? { animationDelay: '1500ms' } : undefined}
        >
