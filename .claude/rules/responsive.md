...
- **Landing Page:** Input and button stack vertically on mobile (`flex-col`), sit side-by-side on `sm:` and up. Flat `bg-bg-primary` surface — no canvas, no JS animation; the staggered CSS entrance on each content row carries the page's only motion. Padding: `px-6` on mobile, `px-8` on `sm:`, `px-10` on `lg:`.
