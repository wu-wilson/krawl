...
- **Landing Page:** Input and button stack vertically on mobile (`flex-col`), sit side-by-side on `sm:` and up. Flat `bg-bg-primary` surface — no canvas. On first mount the spider logo crawls in along a curved bezier path via WAAPI (see `LandingPage.tsx`); the easing decelerates near the end for a soft landing. The legs are also driven by WAAPI during the entrance, slowing in sync with the body and damping to rest before the CSS idle leg-wave resumes. The path's last segment approaches the end from directly below so the spider lands upright at its natural flex-row position. `SpiderLogo` uses `React.forwardRef` so the ref attaches to its `inline-flex` outer box (44×44, no wrapper). On arrival, the rest of the page enters together — wordmark from the left (`animate-landing-from-left`), tagline + URL input + example chips from below (`animate-landing-from-bottom`). The entire entrance plays at most once per page load (`hasPlayedEntrance` module-level flag); return visits via the Navbar logo render the final state instantly. `prefers-reduced-motion: reduce` also short-circuits. Padding: `px-6` on mobile, `px-8` on `sm:`, `px-10` on `lg:`.
